...
const ItemCheckout = (props) => {
  ...
  const validateAddressBeforeCheckout = () => {
    if (orderType === 'delivery') {
      // Check if an address is selected
      if (!address) {
        toast.error(t('Please select a delivery address to continue'));
        return false;
      }
      
      // Check if it's a saved address with an ID (for logged-in users)
      if (token && (!address.id || address.id === undefined)) {
        toast.error(t('Please select a saved address from your address list'));
        return false;
      }
      
      // Check if required address fields are present
      if (!address.address || 
          (token && !address.contact_person_name) || 
          (token && !address.contact_person_number)) {
        toast.error(t('Please select a complete delivery address with contact information'));
        return false;
      }
    }
    return true;
